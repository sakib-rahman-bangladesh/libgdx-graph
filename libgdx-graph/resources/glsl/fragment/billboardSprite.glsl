vec3 billboardSprite(vec3 position, vec2 size, vec2 anchor, float rotation) {
    vec3 result_cameraRight = normalize(cross(u_cameraDirection, u_cameraUp));
    vec3 result_cameraUp = normalize(u_cameraUp);

    float result_xAdjust = size.x * (a_texCoord0.x - anchor.x);
    float result_yAdjust = size.y * (a_texCoord0.y - anchor.y);
